...
path1='/ssd/hongyang/2019/encode_imputation/data/bigwig/'
...
print('label_train',dict_label_train.keys())
print('label_vali',dict_label_vali.keys())
print('feature_common',dict_feature_common.keys())
print('feature_train',dict_feature_train.keys())
print('feature_vali',dict_feature_vali.keys())
...
def generate_data(batch_size, if_train):
    ...
    while True:
        ...
        while b < batch_size:
            ...
            start=np.random.randint(0,chr_len25[the_chr] - size25 - 1,1)
            end=start + size25
            ## 1. label
            label=np.zeros((len(list_label) , 1), dtype='float32')
            num=0
            #for the_id in dict_label.keys():
            for j in np.arange(len(list_label)):
                the_id=list_label[j]
                label[num,:]=np.array(dict_label[the_id].values(the_chr, int(start+flank), int(start+flank+1))).reshape((1,-1))
                num+=1
            ## 2. image
            image=np.zeros((4 + len(list_feature_common)*2 + len(list_feature)*2, size), dtype='float32')
            # 2.1 dna
            num=0
...
model.fit_generator(
    ...
    steps_per_epoch=int(num_sample // (batch_size)), nb_epoch=10,
