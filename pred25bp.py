...
from util.seq_to_hot import seq_to_hot
from util.auc import score_record, calculate_auc
from util.augmentation import pool_avg_2
...
path1='/ssd/hyangl/2019/encode_imputation/data/bigwig/' 
...
#    list_chr=['chr1']
    ...
    for the_chr in list_chr:
        print(the_chr)
        output_all=np.zeros((len(dict_label_test), int(chr_len_cut[the_chr]/25.0)))
        count_all=np.zeros((len(dict_label_test), int(chr_len_cut[the_chr]/25.0)))
        ...
        for i in index: # all start positions
            ...
            ## 2. image
            image=np.zeros((4 + len(dict_feature_common)*2 + len(dict_feature_test)*2 , size*batch), dtype='float32')
            # 2.1 dna
            num=0
            ...
            output1=np.reshape(output1,(size25*batch, len(dict_label_test))).T
        ...
        # add tail
        output_all=np.hstack((output_all, np.zeros((len(dict_label_test),1))))
